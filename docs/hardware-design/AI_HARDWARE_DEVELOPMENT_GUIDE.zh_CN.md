...
ESP32-C3 没有 EXT0/EXT1 唤醒。`bsp_button_arm_wakeup()` 把共用的 ADC 节点配置为**低电平**深睡唤醒源，三个键中任意一个都能唤醒；必须检查返回值，因为唤醒源没装上就等于设备睡下去再也醒不来。`esp_deep_sleep_enable_gpio_wakeup()` 的第一个参数是引脚**位掩码**，不是引脚号。`bsp_button_any_pressed()` 报告当前读数是否落在任一按键窗口内；按键唤醒的应用需要它来等这一次按下松开，因为启动时仍被按住的按键会被 button 组件镜像成一次正常点击。
