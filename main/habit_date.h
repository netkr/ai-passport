...
// 月游标(记录页整月网格用)
//
// 月序号 = year * 12 + (month - 1):单调递增,所以"比较先后"就是比较大小,
// "夹取范围"也只是一次加减。UI 只持有这一个整数,年月由它反推 —— 这样
// "上一个月/下一个月"不会出现跨年时月份越界或年份不跟着走的问题。
// ---------------------------------------------------------------------------

// 该日所在月份的月序号。
int32_t habit_month_index_of_day(int32_t day);

// 月序号对应的"当月 1 号"的日序号(habit_date_to_days 的刻度)。
// 网格里第 c 格对应的日期 = 1 号 + (c - 1 号的星期偏移)。
